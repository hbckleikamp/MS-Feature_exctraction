...
dinofile="C:/Metabolomics/Dinosaur/Dinosaur hills/B004241_Bp_PRC-2296_Remaut_shotgun_1_20180722184121.hills.csv"
...
dino=pd.read_csv(dinofile,sep="\t")
dino=dino.groupby("patternId").nth(0)
dino=dino.sort_values(by="rtApex")
dino_RTs=dino.rtApex

#dino["mz"]=(dino["monoisoMz"]+charge_mass*dino["charge"])/dino["charge"]
#%% 2. Read mzML
...
ms1_RTs=np.array([spec.getRT() for spec in ms1])/60 #important: dinosaur rt is in minutes not in seconds
...
mdino=dino[["monoisoMz","charge",'patternId']].merge(fres,left_index=True,right_index=True,how="right")
mdino["patternId"]=mdino["patternId"].astype(str)
...
cdino["patternId"]=mdino[mdino["c"]!=-1].groupby("c")["patternId"].apply(lambda x: " ".join(list(x)))
cdino=pd.concat([mdino[mdino["c"]==-1],cdino])


#clustered dinosaur features

cdino.to_csv(mzML_file.replace(".mzML",'dinosaur_isotopologue.tsv'),sep="\t")

#%% Write outputs

# Write clustered dinosaur features
...
max_Sirius_mass=1000 #otherwise runs out of RAM and becomes too slow?
sir=cdino.copy()
sir["monoisoMz"]=sir["monoisoMz"]*sir["charge"]-sir["charge"]*charge_mass+charge_mass
sir=sir[sir["monoisoMz"]<=max_Sirius_mass]
...
for _,r in sir.iterrows():
    ...
    if test:
        if len(ls)>test: #test files with 100 entries
            break
    
    pairs=np.vstack([r[mcols].dropna().values,r[icols].dropna().values]).T.astype(str)
    jpairs=[" ".join(i) for i in pairs]
    m=pairs[0,0]
...
ls="\n".join(ls)+"\n"
with open(Path(mzML_file).stem+"max_mass_"+str(max_Sirius_mass)+"_Dinosaur2Sirius.mgf","w") as o:
    o.write(ls)
    


#%% Write Buddy (doesnt work, should be run directly inside of Buddy)


# ls=[]
# for _,r in cdino.iterrows():
...
# ls="\n".join(ls)+"\n"
# with open(Path(mzML_file).stem+"_Dinosaur2Buddy.mgf","w") as o:
#     o.write(ls)
